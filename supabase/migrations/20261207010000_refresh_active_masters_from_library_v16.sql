-- Bring the v16 catalogue to the ACTIVE `report_templates` rows that adopted
...
-- this release did.
--
-- ## What v16 changes in a master, and why it is safe to carry forward
--
-- Two geometry corrections on the Investment Compass's fifty masters. Neither
-- adds, removes or re-binds a block; both change a value inside one.
--
--   * The verdict heading's `headingSize`, on the 27 of 50 masters where the
--     longest publishable headline set past the two lines the block reserves —
--     which is why `$1,975,000` and `$850` were struck through by the heading
--     above them on the 42 Patya Circuit report. The block's own HEIGHT is
--     unchanged, so nothing else on the page moves and no page can gain or
--     lose one.
--
--   * The report pages' running-head marker, on the 39 masters that draw a
--     running head rather than a navigation rail: `Part 05 · Report` on every
--     page of the body becomes the part number plus the chapter the page is
--     in. A binding change inside one text block.
--
-- A row is refreshed only where it is PROVEN an unedited copy of what the
-- library last published, so a tenant who has adjusted either of those values
...
with adopted as (
  ...
  left join public.template_library_release_baselines b
    on b.entry_id = e.id
   and b.release = '20261207000000_seed_template_library_v16_verdict_and_running_head'
  where t.is_active
...
-- nowhere else, so a deferred master can never read as though it received v16.
...
set
  ...
  config = jsonb_set(
    ...
    to_jsonb('20261207000000_seed_template_library_v16_verdict_and_running_head'::text)
  ),
  updated_at = now()
from _v16_classified c
...
--   where release = '20261207000000_seed_template_library_v16_verdict_and_running_head'
...
--     and s.migration = '20261207010000_refresh_active_masters_from_library_v16'
