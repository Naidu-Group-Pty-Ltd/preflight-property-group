-- @effect: select 1 from public.template_master_refresh_decisions where release = '20261223090000_seed_template_library_v22_floor_plan'
...
-- lone SELECT.
-- Bring the v22 catalogue to the ACTIVE `report_templates` rows that adopted
-- from the library — and to no others.
--
-- Identical in mechanism to the v15 refresh (`20261204030000`) and to v18's,
-- v19's, v20's and v21's, which is the point: the classification, the
...
--
-- ## What v22 changes in a master, and why it is safe to carry forward
--
-- The 50 Investment Compass masters, and nothing else: the other nine
-- formats' 450 masters and the 43 voice templates are byte-identical,
-- verified by parsing both seed files and comparing all 543 rows.
--
-- The owner asked on 25 Sep 2026 for a new build's floor plan in its report
-- ("inserting the design and floor plan if applicable"). A new build's
-- brochure usually carries the plan the house will be built to, and the
-- report's author can now choose it from the brochure they upload.
--
--   * Each of the 50 gains two pages, "Floor plan" and "Floor plan,
--     continued", after the cover, the contents and any photographic plates
--     and before the executive dashboard. Each is conditional on its own
--     plan (`property.floorPlans[0]` and `[1]`), so a report without a plan
--     loses the page rather than printing an empty sheet.
--   * The plan is drawn whole (`contain`), never cropped, with the family's
--     own section heading above it and a three-line title block beneath it:
--     not to scale, where it came from, and to check it against the contract
--     drawings.
--   * Every existing page keeps its id, its blocks and its position; 44
--     masters gain 10 blocks and 6 gain 8 (their heading draws no
--     standfirst). `page_count` rises by 2 and `required_bindings` gains
--     `property.floorPlans.0` and `.1`. No other column changes.
--
-- A report without a plan draws exactly what it drew before: that is pinned
-- by `investmentCompassFloorPlan.spec.ts`, which renders every one of the 50
-- with and without the two sheets through the production renderer and
-- requires the output to be byte-identical.
--
-- Every plan is the report's own property's. The binding reads only what the
-- report broker returns, and the broker returns only what was filed under
-- the report after the server judged it a floor plan, from a brochure page
-- naming the report's own address (PROPERTY_PHOTOGRAPHS.md §8).
...
drop table if exists _v22_classified;
create temporary table _v22_classified as
with adopted as (
  select
    t.id as template_id,
    e.id as entry_id,
  ...
  left join public.template_library_release_baselines b
    on b.entry_id = e.id
   and b.release = '20261223090000_seed_template_library_v22_floor_plan'
  where t.is_active
...
-- nowhere else, so a deferred master can never read as though it received v22.
...
set
  ...
  config = jsonb_set(
    ...
    to_jsonb('20261223090000_seed_template_library_v22_floor_plan'::text)
  ),
  updated_at = now()
from _v22_classified c
...
--   where release = '20261223090000_seed_template_library_v22_floor_plan'
...
--     and s.migration = '20261223100000_refresh_active_masters_from_library_v22'
