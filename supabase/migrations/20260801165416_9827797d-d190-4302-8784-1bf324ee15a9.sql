...
begin
  ...
  where resolved_at is null and (
    (alert_key='cron_stale' and exists(select 1 from public.market_updates_automation_runs where job_name='market-updates-ingest-hourly' and dispatch_status='dispatched' and scheduled_at>=now()-interval '2 hours'))
    or (alert_key='source_failures' and not exists(select 1 from public.market_sources where registry_status='canonical' and enabled and consecutive_failures>=3))
    or (alert_key='publication_stale' and exists(select 1 from public.market_updates where status='published' and archived_at is null and ingested_at>now()-interval '72 hours'))
  );
end;
$$;
