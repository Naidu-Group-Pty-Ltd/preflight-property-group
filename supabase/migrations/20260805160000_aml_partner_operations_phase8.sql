-- Operations, reporting and readiness — Phase 8 of the AML/CTF
-- partner/reliance domain.
--
-- Phase 8 is almost entirely read paths over structures Phases 1–7 already
-- created (queues, registers, management reporting and readiness live in
-- aml-reliance ops + the Command Center UI). The only new schema is the
-- operational-target configuration:
--
--   aml.partner_sla_targets — warn/escalate ages and a responsible role per
--   operational queue. These are RECORDED OPERATIONAL TARGETS the MLRO can
--   tune — they are not, and are never presented as, statutory deadlines.
--   Partner-owned queues carry responsible_role='partner_organisation':
--   the originating MLRO is never assigned a partner's own decision.
--
-- ROLLBACK:
--   DROP TABLE IF EXISTS aml.partner_sla_targets;
--   DELETE FROM public.feature_flags WHERE key = 'aml_partner_operations_reporting';
...
-- Feature flag (default OFF; enabling is an operator decision). Gates the
-- queue/register/report operations and their UI. The readiness/preflight
-- operation is deliberately NOT gated: it is the read-only check an
-- operator runs BEFORE enabling anything, reports recorded configuration
-- and database-verifiable facts only, and never claims deployment state.
