-- Stamp duty rate cache: from "whatever the scraper last produced" to a
-- reviewed schedule store.
--
-- The table was created to hold brackets scraped from eight revenue office
-- pages and served as `data_quality = 'live'`. Nothing ever read it, and the
-- scraper never produced a usable bracket, so the rows sat holding a fourth
-- divergent copy of the rates -- one that disagreed with the three copies in
-- application code and with the revenue offices themselves.
--
-- The product now reads this table (see _shared/stampDuty/scheduleStore.ts), so
-- its contract has to be worth trusting:
--
--   built_in     the schedule shipped in code; served
--   override     a human-published correction; served, after validation
--   needs_review a verification sweep found something it could not confirm;
--                deliberately NOT served, held for a person to look at
--
-- A scrape can move a row to needs_review. Only a person can create an
-- override. That asymmetry is the point: these numbers go on client invoices.

-- 1. The schedule itself. `brackets` held {threshold, base, rate} triples, which
--    cannot express a flat band (VIC, ACT), a formula (NT), a separate
--    owner-occupier scale (VIC, QLD, ACT), a premium scale (NSW), or any
--    concession at all.
...
-- 2. Retire the old vocabulary. Anything previously marked 'live' was written by
--    the retired scraper and must not be served.
...
-- 3. Reseed every jurisdiction from the shipped schedules.
--
--    Generated by scripts/stampDuty/generate-seed.ts -- do not hand-edit. The
--    figures were taken from each revenue office on 2026-08-10 and every band
--    boundary is checked for continuity by src/utils/__tests__/stampDutyCalculator.spec.ts.
--
--    What this replaces was wrong in ways worth naming, because the same
--    mistakes are easy to reintroduce: NSW and ACT a financial year or more
--    behind, South Australia missing its $200k-$250k band entirely, Tasmania
--    wrong in three bands, WA's base amounts and top rate wrong, and the
--    Northern Territory -- which is quadratic below $525,000 -- modelled as
...
-- 4. Expiry no longer means "downgrade to fallback" -- there is no fallback tier
--    any more, and silently changing what a row means is how the old table came
--    to be untrustworthy. An expired override simply stops being served, which
--    the reader already enforces; this marks it for review so somebody knows.
...
-- 5. Weekly verification sweep.
--
--    Weekly rather than daily because there is nothing to catch in between:
--    duty schedules change on a budget announcement or at an indexation date,
--    not continuously, and each run costs eight metered scrapes. What the sweep
--    is really guarding against is the failure this programme was built to fix
--    -- a schedule quietly outliving its financial year while the product keeps
--    quoting it. NSW and the ACT re-index every 1 July, and from that date the
--    staleness check flags them with no network call at all.
DO $$
BEGIN
...
-- 6. Expired overrides are swept on the same cadence, an hour earlier, so a
--    lapsed override is already back on the shipped schedule by the time the
