-- Archived market news is a reversible lifecycle state, not a deletion queue.
-- This forward-only correction removes the earlier 30-day purge schedule and
-- keeps the compatibility RPC as a safe no-op for any in-flight deployment.
