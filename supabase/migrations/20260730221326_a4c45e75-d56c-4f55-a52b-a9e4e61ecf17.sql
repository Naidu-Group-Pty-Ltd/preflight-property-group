...
DO $$DECLARE t text;BEGIN FOREACH t IN ARRAY ARRAY['cross_portal_feature_definitions','cross_portal_firm_rollouts','cross_portal_rollout_history','cross_portal_dual_read_comparisons','cross_portal_cutover_approvals','cross_portal_reconciliation_runs'] LOOP EXECUTE format('ALTER TABLE public.%I ENABLE ROW LEVEL SECURITY',t);BEGIN EXECUTE format('CREATE POLICY %I ON public.%I FOR ALL TO service_role USING(true) WITH CHECK(true)',t||'_service',t);EXCEPTION WHEN duplicate_object THEN NULL;END;END LOOP;END$$;
