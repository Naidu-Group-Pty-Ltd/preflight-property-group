...
function ReportGenerationProgressInner() {
  ...
  const handleContinueGeneration = useCallback(
    async (reportId: string, isAutoRetry = false) => {
      let driver: ReturnType<typeof newDriverIdentity> | null = null;
      try {
        ...
        /* This panel is the SECOND pump. `useChunkedRegeneration` drives the
           same sections from the Regenerate button, and on 20 Sep 2026 both
           drove 97 Poole Road at once — the production log shows one at
           `last_completed_section=14, 134,392 chars` while the other was
           writing section 8, and the finished document came out 14,247
           characters shorter. `autoRetryInProgressRef` only stops this panel
           racing ITSELF; the claim is what stops it racing the hook, another
        ...
        if (claim.ok === false) {
          ...
          if (!isAutoRetry) {
            toast.info('Already generating', {
              description: `This report is being generated by ${driverHolderLabel(claim.heldBy)}.`,
            });
          }
          driver = null;
          return;
        ...
        for (let call = 0; call < MAX_SECTION_CALLS && !done; call++) {
          ...
          /* Once a call, so the claim measures real progress rather than a
             timer: a pump wedged inside one call lets its lease lapse and the
             report becomes recoverable instead of held for ever. */
          if (driver) heartbeatGenerationDriver(reportId, driver);
        ...
        // Hand the report back so the next driver can take it immediately
        // rather than waiting out the lease.
        if (driver) releaseGenerationDriver(reportId, driver);
      }
    },
  ...
  const scheduleAutoRetry = useCallback(
    (report: ReportProgress) => {
      ...
      /* Somebody is already driving this report — the Regenerate hook in this
         tab, or a pump in another one. `isResumable` says yes to any report
         reading `failed`, which is exactly what the hook writes on its way
         out, so without this a single failure turned into a second concurrent
         pump within `delaySeconds`. */
      if (generationDriverHolder(id)) return;
