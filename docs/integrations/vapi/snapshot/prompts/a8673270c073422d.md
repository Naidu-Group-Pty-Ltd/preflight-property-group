...
* `{{callTitle}}` — "Strategy Session with NPC Services" (hard-set)
...
3. If no contact is found → run `ghl_contact_create` using:
   ...
   * `callerPhone` (mandatory) - Always ensure that the number starts with "+61" if manually provided by the caller
